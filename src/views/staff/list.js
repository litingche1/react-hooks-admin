...
const JobList = () => {
        ...
        columns: [
            ...
            { title: "转正时间", dataIndex: "job_formal_date", key: "job_formal_date" },
            {
                ...
                render: (status, rowData) => {
                    return <Switch loading={switchId === rowData.staff_id} onChange={() => { swithOnChange(rowData) }} checkedChildren="启用" unCheckedChildren="禁用" defaultChecked={rowData.status} />
                }
            },
            {
                ...
                render: (text, rowData) => {
                    return (
                        <div className="inline-button">
                            <Link to={{ pathname: '/index/staff/add', state: { id: rowData.staff_id } }}><Button type="primary" onClick={() => { goPage(rowData.staff_id) }}>编辑</Button></Link>

                            <Button className="ml10" onClick={e => { deleteList(rowData.staff_id) }}>删除</Button>
                        </div>
                    )
                }
            },
    ...
    return (
        <Fragment>
            ...
            <TableCommon rowKey={record => record.staff_id} cref={table} batchButton={true} config={tableConfig}></TableCommon>
        </Fragment>
    )
}
